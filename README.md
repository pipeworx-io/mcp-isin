...
`tools/list` at `https://gateway.pipeworx.io/isin/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/validate_isin \
  -H 'Content-Type: application/json' \
  -d '{"isin":"US0378331005"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/validate_isin`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
